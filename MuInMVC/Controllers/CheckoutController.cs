...
namespace MuInMVC.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Checkout(CheckoutVM userInfo)
        {
            var cart = HttpContext.Session.Get<List<AddToCartVM>>("GioHang");
            string cartData = JsonConvert.SerializeObject(cart);
            ...
            RequestCheckout newRequestCheckout = new RequestCheckout
            {
                cart = cartItemReponses,
                userInfo = userInfo,
            };

            string data = JsonConvert.SerializeObject(newRequestCheckout);
            bool resultCheckout = _checkoutService.CheckOut(token, data);
            if (resultCheckout)
            {
